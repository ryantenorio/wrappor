...
export class Wrappor implements Encoder {
  ...
  private signal = (v: string, cohort: number, numHashes: number, bloomSize: number) => {
  ...
  private doSignal = (v: string, cohort: number, numHashes: number, bloomSize: number) => {
    let bloomBits = this.signal(v, cohort, numHashes, bloomSize)
    let bloom = this.buildBloom(bloomBits)
    return bloom
  }
  
  private getPRRMasks = (word: string, secret: string, f: number, numBits: number) => {
  ...
  private doIRR = (prr: number, randProvider: RRandom, p: number, q: number, s: number) => {
    let irr = 0
  ...
  encode = (word: string) => {
    let irr = 0
    let bloom = this.doSignal(word, this.clientCohort, this.config.hashes, this.config.bloomBits)
    let prr = this.doPRR(bloom, this.clientSecret, this.config.fProb, this.config.bloomBits)
    irr = this.doIRR(prr, this.randGenerator, this.config.pProb, this.config.qProb, this.config.bloomBits)
    return irr
  }
}